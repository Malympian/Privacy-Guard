...
/** Returns the list of URL exception patterns (allow-even-if-matched). */
function getExceptions(sync) {
  return (sync.exceptions ?? []).map((p) => p.trim()).filter(Boolean);
}

/**
 * Derive a stable exception key from a full URL.
 * We use origin+pathname (drop query string / hash) so that
 * "allow /events" applies to all calls to that endpoint regardless of params.
 */
...
async function getInjectDecision(hostname, tabId) {
  ...
  const sharedConfig = {
    ...
    exceptions: getExceptions(sync),   // ← passed to guard.js
  };
...
// ── Block / observe recording ─────────────────────────────────────────────────

async function recordBlock(tabId, url) {
  if (tabId == null) return;

  const { tabStats = {} } = await chrome.storage.session.get({ tabStats: {} });
  const prev = tabStats[tabId] ?? { count: 0, log: [] };
  const entry = { url, at: Date.now() };
  const log = [entry, ...prev.log].slice(0, MAX_LOG);

  tabStats[tabId] = { count: prev.count + 1, log };
  await chrome.storage.session.set({ tabStats });
  await updateActionForTab(tabId);
}

/**
 * Record a network request seen on a tab.
 *
 * Each unique URL is stored as ONE entry (keyed by full URL).
 * Structure:
 *   {
 *     url:             string   — full URL
 *     hits:            number   — how many times seen this page load
 *     via:             string   — "fetch" | "xhr" | "beacon"
 *     matchedPatterns: string[] — blocked-pattern fragments that matched
 *     decision:        "block" | "allow"
 *     reason:          string | null  — first matching pattern, or "guard-blocked"
 *     blocked:         boolean
 *     isExcepted:      boolean  — true if an exception rule overrides a block
...
// ── Clear per-tab data on navigation ──────────────────────────────────────────
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  if (sync.exceptions    === null) patch.exceptions    = [];   // ← new
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "recordObserveReady") {
    recordObserveReady(sender.tab?.id).then(() => sendResponse({ ok: true }));
    return true;
  }

  if (msg.type === "refreshBadge") {
    if (msg.tabId > 0) {
      updateActionForTab(msg.tabId).then(() => sendResponse({ ok: true }));
    } else {
  ...
  /**
   * addException: store an allow-rule so guard.js skips blocking a URL.
   *
   * msg.url     — the full URL that was blocked (pattern derived from it)
   * msg.pattern — if provided, used directly instead of deriving from URL
   *
   * The stored pattern is origin+pathname (no query string), so that
   * all calls to the same endpoint are allowed regardless of params.
   */
  ...
  /**
   * removeException: delete a previously-added allow-rule.
   * msg.pattern — exact string that was stored (as returned by addException).
   */
  ...
  /** getExceptions: return the full exception list for display in the popup. */
