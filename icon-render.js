// icon-render.js — golden lock icon with block-count badge.
// No on/paused/off status dots. Status is shown in the popup only.

const ICON_SIZES = [16, 32, 48, 128];
const imageDataCache = new Map();

/**
 * Draw a golden padlock on an OffscreenCanvas context.
 * Design: deep navy/slate background, warm gold lock body, glowing shackle.
 */
function drawLockIcon(ctx, size) {
  const s = size;
  const cx = s / 2;

  // ── Background: deep blue-grey with subtle radial glow ──────────────────
...
/** Utility: draw a rounded rectangle path (no fill/stroke — caller does that). */
...
/**
 * Draw block count badge in bottom-right corner.
 * Larger and clearer than the previous version.
 */
function drawBlockCount(ctx, size, count) {
  ...
  // Badge radius: bigger relative to icon
  ...
  // Badge fill: vivid red-orange for visibility
...
/**
 * Build ImageData dict for all icon sizes.
 * `status` param is accepted for API compatibility but ignored visually —
 * the icon is always the same golden lock. Status is shown in the popup.
 */
...
/** No-op — no bitmap files to preload anymore. */
